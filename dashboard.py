...
if botao_analisar and not df_dados.empty and modelos_ia:
    with st.spinner('A IA está a consultar padrões em todos os mercados...'):
        ...
        # A IA avalia dinamicamente os 13 mercados
        previsoes_finais = {}
        for mercado, modelo in modelos_ia.items():
            probabilidade = modelo.predict_proba(X_novo)[0][1] * 100
            previsoes_finais[mercado] = probabilidade
            
        # Filtros de Inteligência - Bloqueio de Absurdos
        if (avg_HST + avg_AST) < 7.5:
            previsoes_finais.pop("Over 2.5 FT", None)
            previsoes_finais.pop("Over 3.5 FT", None)
            previsoes_finais.pop("Over 1.5 HT", None)
            
        if (avg_HC + avg_AC) < 9.5:
            previsoes_finais.pop("Cantos: Over 9.5", None)
            previsoes_finais.pop("Cantos: Over 10.5", None)
        ...
        st.info("A IA avaliou as estatísticas intensivas destas equipas comparando-as com milhares de padrões de jogos passados em **13 mercados diferentes**, extraindo as dicas mais seguras.")
