    ...
    if os.path.exists(ficheiro_dados):
        try:
            ...
            if ficheiro_dados.endswith('.csv'):
                try:
                    df = pd.read_csv(ficheiro_dados, usecols=colunas_usar, low_memory=False)
                except ValueError:
                    df = pd.read_csv(ficheiro_dados, low_memory=False)
            elif ficheiro_dados.endswith('.xlsx'):
                df = pd.read_excel(ficheiro_dados)
            else:
                return pd.DataFrame()
            
            # Limpeza e Otimização de Memória RAM
...
# AGORA CHAMA DIRETAMENTE O CSV!
ficheiro_dados = 'BD_A.csv' if is_historico else 'BD_B.xlsx' 

df_dados = carregar_dados(ficheiro_dados)
...
if botao_analisar:
    with st.spinner(f'A processar cenários para {jogo_hoje_casa} vs {jogo_hoje_fora}...'):
        ...
        if aviso_sem_ht:
            st.info("ℹ️ **Nota de Análise:** Esta liga não possui registo de golos ao intervalo. O modelo estimou as probabilidades de HT usando a estatística global (45% dos golos acontecem na 1ª parte).")
