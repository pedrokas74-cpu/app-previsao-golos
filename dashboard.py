...
# --- SECÇÃO: O MEU HISTÓRICO (BD PRÓPRIA) ---
st.sidebar.markdown("---")
st.sidebar.header("📁 O Meu Histórico")
st.sidebar.write("Os teus jogos analisados ficam guardados aqui.")
...
if os.path.exists(ficheiro_historico):
    with open(ficheiro_historico, 'rb') as f:
        st.sidebar.download_button(
            label="📥 Descarregar TODA a Base de Dados",
            data=f.read(),
            file_name="bd_minhas_analises.csv",
            mime="text/csv",
            type="primary"
        )
else:
    st.sidebar.info("Ainda não tens análises guardadas.")
...
if botao_analisar and not df_dados.empty:
    with st.spinner(f'A processar cenários para {jogo_hoje_casa} vs {jogo_hoje_fora}...'):
        ...
        # CRIAR A LINHA DE DADOS A EXPORTAR
        ...
        # --- A MAGIA DA GRAVAÇÃO AUTOMÁTICA ---
        if os.path.exists(ficheiro_historico):
            dados_exportar.to_csv(ficheiro_historico, mode='a', header=False, index=False, sep=';', encoding='utf-8-sig')
        else:
            dados_exportar.to_csv(ficheiro_historico, mode='w', header=True, index=False, sep=';', encoding='utf-8-sig')

        # ==========================================
        # INTERFACE VISUAL
        # ==========================================
        st.success("✅ Jogo analisado e gravado com sucesso na tua Base de Dados (consulta o menu lateral).")
        st.markdown("---")
        ...
        if aviso_sem_cantos: st.warning("⚠️ **Cantos:** Não há histórico de cantos para estas equipas. A usar médias globais.")
        ...
        # Mantemos o botão antigo cá em baixo na mesma para se quiseres descarregar SÓ este jogo específico
        st.markdown("---")
        st.markdown("### 💾 Guardar Análise Singular")
        csv = dados_exportar.to_csv(index=False, sep=';').encode('utf-8-sig')
        st.download_button(
            label="📥 Descarregar Apenas Este Jogo (CSV)",
            data=csv,
            file_name=f"analise_singular_{jogo_hoje_casa}_{jogo_hoje_fora}.csv",
            mime='text/csv'
        )