...
# O DICIONÁRIO PREMIUM (Apenas a tua lista de elite)
DICIONARIO_LIGAS = {
...
def carregar_dados(ficheiro_dados):
    ...
    try:
        ...
        if ficheiro_dados.endswith('.zip'):
            with zipfile.ZipFile(ficheiro_dados, 'r') as z:
                ...
                if not ficheiros_csv:
                    return pd.DataFrame(), "Erro: ZIP vazio ou sem CSV."
            ...
            try:
                ...
                df = pd.read_csv(ficheiro_dados, low_memory=True)
        else:
        ...
        # BLINDAGEM DE FERRO: Remove tudo o que não estiver na nossa lista de Elite!
        df = df[df['Liga_Codigo'].isin(DICIONARIO_LIGAS.keys())].copy()
            
        for col in ['Liga_Codigo', 'Equipa_Casa', 'Equipa_Fora']:
            if col in df.columns:
                df[col] = df[col].astype('category')
        for col in ['Golos_FT_Casa', 'Golos_FT_Fora', 'Golos_HT_Casa', 'Golos_HT_Fora']:
            if col in df.columns:
