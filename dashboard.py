...
st.sidebar.markdown("---")
botao_analisar = st.sidebar.button("Executar Análise de Inteligência", type="primary", use_container_width=True)
n_simulacoes = st.sidebar.slider("Simulações:", 1000, 50000, 10000)

st.sidebar.markdown("---")
st.sidebar.header("3. Comparador Manual (Opcional)")
mercados_opcoes = [
...
mercado_alvo = st.sidebar.selectbox("Testar mercado específico:", mercados_opcoes)
odd_casa_apostas = st.sidebar.number_input("Odd da Casa de Apostas:", min_value=1.01, max_value=20.0, value=1.90, step=0.01)

# ==========================================
# MOTOR MATEMÁTICO (xG + RECONHECIMENTO DE PADRÕES)
# ==========================================
if botao_analisar and not df_dados.empty:
    with st.spinner(f'A cruzar dados de xG, Faltas e Cantos para {jogo_hoje_casa} vs {jogo_hoje_fora}...'):
        ...
        # ==========================================
        # 🤖 O CÉREBRO: O ALGORITMO DE SUGESTÃO
        # ==========================================
        # 1. Tirar o mercado 1X2 da lista de opções
        mercados_alternativos = {k: v for k, v in probs.items() if "1X2" not in k}
        
        # 2. Aplicar Padrões de Inteligência (Filtros de xG e Intensidade)
        total_xg = xg_casa + xg_fora
        intensidade_total = media_faltas_casa + media_faltas_fora
        remates_total = media_remates_baliza_casa + media_remates_baliza_fora

        # Se não há faro de golo, remover recomendações agressivas de Golos
        if total_xg < 2.3:
            mercados_alternativos.pop("Over 2.5 FT", None)
            mercados_alternativos.pop("Over 3.5 FT", None)
            mercados_alternativos.pop("Over 1.5 HT", None)
            
        # Se as equipas não rematam nem fazem faltas, não sugerir Overs de Cantos Altos
        if intensidade_total < 22 or remates_total < 8:
            mercados_alternativos.pop("Cantos: Over 9.5", None)
            mercados_alternativos.pop("Cantos: Over 10.5", None)
            
        # Se as equipas são muito ofensivas, não sugerir Unders
        if total_xg > 2.8:
            mercados_alternativos.pop("Under 2.5 FT", None)
            mercados_alternativos.pop("Under 3.5 FT", None)

        # 3. Ordenar as sugestões que sobraram pelas que têm MAIOR PROBABILIDADE
        sugestoes_ordenadas = sorted(mercados_alternativos.items(), key=lambda item: item[1], reverse=True)
        top_2 = sugestoes_ordenadas[:2] # Apanha apenas as 2 melhores
        ...
        # --- A GRANDE CAIXA DO VEREDICTO ---
        st.markdown("## 🤖 O Veredicto do Algoritmo")
        st.info(f"Cruzei o xG, a intensidade de jogo, as faltas e {n_simulacoes} simulações. Excluí o mercado de Match Odds (1X2) e encontrei os seguintes padrões de aposta mais seguros para este jogo:")
        
        col_sug1, col_sug2 = st.columns(2)
        if len(top_2) >= 1:
            col_sug1.metric(label="⭐ Melhor Aposta (Segurança)", value=f"{top_2[0][0]}", delta=f"{top_2[0][1]:.1f}% Probabilidade")
        if len(top_2) >= 2:
            col_sug2.metric(label="⭐⭐ Dica Secundária (Valor)", value=f"{top_2[1][0]}", delta=f"{top_2[1][1]:.1f}% Probabilidade")
        ...
        if aviso_dados_avancados:
            ...
            c3.metric(label="Média Faltas (Intensidade)", value=f"{intensidade_total:.1f}")
            c4.metric(label="Remates à Baliza/Jogo", value=f"{remates_total:.1f}")
            
        st.markdown("---")
        st.markdown(f"### 📈 Teste Manual: {mercado_alvo}")
        col_odd1, col_odd2, col_odd3 = st.columns(3)
        col_odd1.metric("Odd da Casa de Apostas", f"{odd_casa_apostas:.2f}")
        col_odd2.metric("Odd Justa (Modelo)", f"{odd_justa:.2f}")
        ...
        st.markdown("---")
        st.markdown("### 🏁 Mercados de Golos (HT e FT)")
        cf1, cf2, cf3, cf4 = st.columns(4)
        cf1.metric("Over 0.5 HT", f"{probs['Over 0.5 HT']:.1f}%")
        cf2.metric("Over 1.5 HT", f"{probs['Over 1.5 HT']:.1f}%")
        cf3.metric("Over 1.5 FT", f"{probs['Over 1.5 FT']:.1f}%")
        cf4.metric("Over 2.5 FT", f"{probs['Over 2.5 FT']:.1f}%")
        
        cx1, cx2, cx3 = st.columns(3)
        cx1.metric("Under 2.5 FT", f"{probs['Under 2.5 FT']:.1f}%")
        cx2.metric("Over 3.5 FT", f"{probs['Over 3.5 FT']:.1f}%")
        cx3.metric("Under 3.5 FT", f"{probs['Under 3.5 FT']:.1f}%")